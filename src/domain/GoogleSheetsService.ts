  ...
  static async syncAllToGoogleSheets(
    ...
    try {
      ...
      if (response.ok || response.type === 'opaque') {
        ...
        return {
          isSuccess: true,
          message: `Berhasil mengekspor matriks ke tab "Matriks HD - ${matrixData.formattedMonthTitle}" di Google Sheets! (${assignments.length} jadwal perawat${docMsg}). Tab bulan sebelumnya tetap tersimpan utuh.`,
...
function doGet(e) {
  try {
    ...
    // 4. Baca Jadwal: Prioritas baca tab khusus bulan (misal 'Matriks HD - September 2026') atau 'Matriks Jadwal HD'
    var assignments = [];
